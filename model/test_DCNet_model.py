...
class grounding_model(nn.Module):
    ...
    def cal_corr_feat(self, input1, input2):
        ...
        for ii in range(scale):
            ...
            corr_feat.append(torch.cat([input1[ii], input1_attn[ii]],1)) # b x (2c) x h x w
            


        for ii in range(len(corr_feat)):
            corr_feat[ii] = self.corr_conv[ii](corr_feat[ii])
            corr_feat[ii] = F.normalize(corr_feat[ii], p=2, dim=1)

        return corr_feat

    def forward(self, image, word_id, word_mask, n_frame=5):
        ## Visual Module
        ## [1024, 13, 13], [512, 26, 26], [256, 52, 52]
        batch_size = int(image.size(0) / n_frame)
        ...
        fvisu[0] = fvisu[0].contiguous().view(batch_size, n_frame, fvisu[0].shape[-3], fvisu[0].shape[-2], fvisu[0].shape[-1])
        fvisu[1] = fvisu[1].contiguous().view(batch_size, n_frame, fvisu[1].shape[-3], fvisu[1].shape[-2], fvisu[1].shape[-1])
        fvisu[2] = fvisu[2].contiguous().view(batch_size, n_frame, fvisu[2].shape[-3], fvisu[2].shape[-2], fvisu[2].shape[-1])

        center_idx = int(n_frame / 2)

        # split the input
        input1 = []
        corr_feat_set = []
        for ii in range(len(raw_fvisu)):
            input1.append(fvisu[ii][:,center_idx])
        ...
        for idx in range(n_frame):
            ...
            input2 = []
            for ii in range(len(raw_fvisu)):
                input2.append(fvisu[ii][:,idx])
                
            
            corr_feat_set.append(self.cal_corr_feat(input1, input2))

        # corr_feat: [[b x c1 x h1 x w1, b x c2 x h2 x w2, b x c3 x h3 x w3], [b x c1 x h1 x w1, b x c2 x h2 x w2, b x c3 x h3 x w3], ...]
        # calculate the averaged feature
        corr_feat = []
        for ii in range(len(raw_fvisu)):
            temp_feat = []
            for idx in range(len(corr_feat_set)):
                temp_feat.append(corr_feat_set[idx][ii].unsqueeze(0))
            # temp_feat: [1 x b x c1 x h1 x w1, 1 x b x c1 x h1 x w1, 1 x b x c1 x h1 x w1, 1 x b x c1 x h1 x w1]
            temp_feat = torch.cat(temp_feat, dim=0) # (n_frame - 1) x b x c x h x w
            temp_feat = torch.mean(temp_feat, dim=0) # b x c x h x w
            corr_feat.append(temp_feat)
        ...
        if self.training:
            ...
            return outbox, sim_score, loc_score, corr_feat, only_obj
