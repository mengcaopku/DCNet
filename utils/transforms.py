...
def random_affine_set(imgs, mask, targets, degrees=(-10, 10), translate=(.1, .1), scale=(.9, 1.1), shear=(-2, 2),
                  borderValue=(123.7, 116.3, 103.5), all_bbox=None):
    border = 0  # width of added border (optional)
    height = max(imgs[0].shape[0], imgs[0].shape[1]) + border * 2
    ...
    # a += random.choice([-180, -90, 0, 90])  # 90deg rotations added to small rotations
    s = random.random() * (scale[1] - scale[0]) + scale[0]
    R[:2] = cv2.getRotationMatrix2D(angle=a, center=(imgs[0].shape[1] / 2, imgs[0].shape[0] / 2), scale=s)

    # Translation
    T = np.eye(3)
    T[0, 2] = (random.random() * 2 - 1) * translate[0] * imgs[0].shape[0] + border  # x translation (pixels)
    T[1, 2] = (random.random() * 2 - 1) * translate[1] * imgs[0].shape[1] + border  # y translation (pixels)
    ...
    for ii in range(len(imgs)):
        imgs[ii] = cv2.warpPerspective(imgs[ii], M, dsize=(height, height), flags=cv2.INTER_LINEAR,
                                borderValue=borderValue)  # BGR order borderValue
    if mask is not None:
        maskw = cv2.warpPerspective(mask, M, dsize=(height, height), flags=cv2.INTER_NEAREST,
                                  borderValue=255)  # BGR order borderValue
    ...
    if type(targets)==type([1]):
        ...
        for bbox in targets:
            targetlist.append(wrap_points(bbox, M, height, a))
        return imgs, maskw, targetlist, M
    elif all_bbox is not None:
        targets = wrap_points(targets, M, height, a)
        for ii in range(all_bbox.shape[0]):
            all_bbox[ii,:] = wrap_points(all_bbox[ii,:], M, height, a)
        return imgs, maskw, targets, all_bbox, M
    elif targets is not None:   ## previous main
        targets = wrap_points(targets, M, height, a)
        return imgs, maskw, targets, M
    else:
        return imgs
