...
class ReferDataset(data.Dataset):
    ...
    def __init__(self, data_root, split_root='data', dataset='referit', imsize=256,
        ...
        elif self.dataset == 'OTB':
            self.dataset_root = osp.join(self.data_root, 'OTB')
            self.im_dir = osp.join(self.dataset_root, 'OTB')
        else:   ## refcoco, etc.
    ...
    def pull_item(self, idx):
        if self.dataset == 'flickr' or self.dataset == 'VID' or self.dataset == 'VID_noun' or self.dataset == 'gref' or self.dataset == 'OTB':
            img_file, bbox, phrase = self.images[idx]
        else:
            img_file, _, bbox, phrase, attri = self.images[idx]
        ## box format: to x1y1x2y2
        if not (self.dataset == 'referit' or self.dataset == 'flickr' or self.dataset == 'VID' or self.dataset == 'VID_noun' or self.dataset == 'OTB'):
            bbox = np.array(bbox, dtype=int)
            bbox[2], bbox[3] = bbox[0]+bbox[2], bbox[1]+bbox[3]
        else:
            bbox = np.array(bbox, dtype=int)

        if self.dataset == 'VID' or self.dataset == 'VID_noun' or self.dataset == 'OTB':
            self.im_dir = ''
    ...
    def __getitem__(self, idx):
        ...
        if self.testmode:
            return img, np.array(word_id, dtype=int), np.array(word_mask, dtype=int), \
                ...
                np.array(dw, dtype=np.float32), np.array(dh, dtype=np.float32), self.images[idx][0],ori_phrase
        else:
            return img, np.array(word_id, dtype=int), np.array(word_mask, dtype=int), \
            np.array(bbox, dtype=np.float32), ori_phrase
